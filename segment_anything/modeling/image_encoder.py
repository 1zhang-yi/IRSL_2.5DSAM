...
from .cross_slice_attention import MultiHeadedCrossSliceAttentionModule
...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.cross_slice_attentions = MultiHeadedCrossSliceAttentionModule(
        ...
        #     CLA = MultiHeadedCrossSliceAttentionModule(
        #         input_channels=embed_dim,
        #         heads=1,
        #         input_size=(32, 32),
        #         batch_size=12
        #     )
        #     self.cross_slice_attentions.append(CLA)
    ...
    def forward(self, x: torch.Tensor, x_res: torch.Tensor) -> torch.Tensor:
        ...
        x = self.cross_slice_attentions(x)

        x = self.neck(x.permute(0, 3, 1, 2))  # [b, c, h, w], [1, 256, 64, 64]
        x_res = self.res_embed(x_res)
