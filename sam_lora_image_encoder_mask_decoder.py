...
class LoRA_Sam(nn.Module):
    ...
    def __init__(self, sam_model: Sam, r: int, lora_layer=None):
        ...
        for param in sam_model.image_encoder.cross_slice_attentions.parameters():
            param.requires_grad = True
        for param in sam_model.image_encoder.res_embed.parameters():
            param.requires_grad = True
    ...
    def save_lora_parameters(self, filename: str) -> None:
        ...
        for key, value in state_dict.items():
            if 'cross_slice_attentions' in key:
                cross_slice_attentions[key] = value
            if 'res_embed' in key:
                res_embed[key] = value
        ...
        merged_dict = {**a_tensors, **b_tensors, **sa_a_tensors, **sa_b_tensors, **cti_a_tensors, **cti_b_tensors,
                       **cit_a_tensors, **cit_b_tensors, **cross_slice_attentions, **res_embed, **prompt_encoder_tensors,
                       **mask_decoder_tensors, **fa_ti_tensors}
    ...
    def load_lora_parameters(self, filename: str) -> None:
        ...
        # load res embed
        res_embed_keys = [k for k in sam_keys if 'res_embed' in k]
        res_embed_values = [state_dict[k] for k in res_embed_keys]
        res_embed_new_state_dict = {k: v for k, v in zip(res_embed_keys, res_embed_values)}
        sam_dict.update(res_embed_new_state_dict)

        # load cross slice attention
        cross_slice_attention_keys = [k for k in sam_keys if 'cross_slice_attentions' in k]
        cross_slice_attention_values = [state_dict[k] for k in cross_slice_attention_keys]
        cross_slice_attention_new_state_dict = {k: v for k, v in zip(cross_slice_attention_keys, cross_slice_attention_values)}
        sam_dict.update(cross_slice_attention_new_state_dict)
